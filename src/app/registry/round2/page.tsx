...
export default function Round2Registry() {
  const [matches, setMatches] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    getMatchesWithScores(2).then(data => {
      setMatches(data);
      setLoading(false);
    });
  }, []);
  ...
  const handleSave = async (match: any) => {
    setSaving(true);
    if (match.team_a) await saveMatchScore(match.id, match.team_a.id, 2, match.score_a);
    if (match.team_b) await saveMatchScore(match.id, match.team_b.id, 2, match.score_b);
    setSaving(false);
    ...
    <div className="space-y-8 animate-in fade-in duration-1000 max-w-7xl mx-auto">
      <div>
        <h1 className="text-3xl md:text-4xl font-cinzel text-gold-accent flex items-center gap-4">
          <Gavel size={32} />
          Round II Scoring (Finals)
        </h1>
        <p className="text-parchment/60 font-inter mt-2 text-sm">
          Enter and lock judges' scores for the final round.
        </p>
      ...
      <div className="space-y-6">
        ...
        {matches.length === 0 && (
          <div className="p-12 text-center text-parchment/50 font-inter text-sm border border-gold-muted/30 bg-court-charcoal/50">
            No Round II matches scheduled yet.
          </div>
        )}
      </div>
    </div>
